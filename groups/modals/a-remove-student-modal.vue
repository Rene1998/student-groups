<template>
	<b-modal ref="modal-remove-student-from-group" ok-only hide-title ok-title="Zavrieť" size='lg'
		@ok="$emit('remove', itemToRemove || student)">
		<template #modal-header="{ close }">
			<h5>Naozaj chcete vymazať tento profile?</h5>
			<svg-cross-icon @click="close()"/>
		</template>
		<div class="m-5">
			<p>Ste si istý, že chcete odstraniť
				<b v-if="itemToRemove" class="fw-bold">{{itemToRemove.user.name}} {{itemToRemove.user.surname}}</b>
				<b v-if="student" class="fw-bold">{{student.user.name}} {{student.user.surname}}</b>
				z triedy
				<b v-if="group" class="fw-bold">{{group.name }} {{group.second_name }}</b>
			</p>
		</div>
...
export default {
	...
	methods: {
		showModal(e) {
			this.itemToRemove = e
			this.$nextTick(() => {
				this.$refs['modal-remove-student-from-group'].show()
			})
		}
	}
}
